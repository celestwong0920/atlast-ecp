...
async def _get_local_pending_batches() -> list[dict]:
    """Fetch pending + retry_queued batches from local DB (direct SDK uploads)."""
    try:
        session = await get_session()
        if session is None:
            return []
        ...
        async with session:
            result = await session.execute(
                select(Batch).where(
                    or_(Batch.status == "pending", Batch.status == "retry_queued")
                ).order_by(Batch.created_at).limit(50)
            )
...
async def _anchor_super_batch(batches: list[dict], _anchor_start: float) -> dict:
    ...
    # Single EAS attestation — failure marks all batches for retry
    try:
        ...
        # Mark all local batches for retry
        from ..services.anchor_coordinator import mark_batch_retry
        for b in batches:
            if b.get("_source") == "local":
                await mark_batch_retry(b["batch_id"], f"super_batch_eas_failed: {e}")
...
async def _anchor_pending():
    """
    Core anchor logic — with coordinator lock, gas check, and retry management.
    
    Flow:
      1. Acquire lock (process + DB distributed)
      2. Check gas balance
      3. Fetch pending + retry_queued batches
      4. Filter out exhausted retries
      5. Anchor (super-batch or individual)
      6. On failure: mark batch for retry (not stub fallback)
      7. Release lock
    """
    import time as _time
    from ..services.anchor_coordinator import (
        try_acquire_lock, release_lock, check_gas_balance,
        mark_batch_retry, should_retry_batch, _anchor_lock,
    )

    # Step 1: Acquire distributed lock
    can_proceed = await try_acquire_lock()
    if not can_proceed:
        return {"processed": 0, "anchored": 0, "errors": 0, "skipped": "lock_held"}

    async with _anchor_lock:
        try:
            _anchor_start = _time.time()

            # Step 2: Check gas balance (production only)
            gas_ok, balance_wei = await check_gas_balance()
            if not gas_ok:
                logger.error("anchor_paused_low_gas", balance_wei=balance_wei)
                capture_error(
                    RuntimeError(f"Gas critically low: {balance_wei} wei"),
                    {"context": "gas_check", "balance_wei": balance_wei}
                )
                return {"processed": 0, "anchored": 0, "errors": 0, "paused": "low_gas", "balance_wei": balance_wei}
            ...
            # Step 4: Filter out permanently failed batches
            batches = [b for b in batches if should_retry_batch(b)]
            if not batches:
            ...
            for batch in batches:
                try:
                    ...
                    # Step 6: Mark for retry instead of silently failing
                    capture_error(e, {"context": "anchor_batch", "batch_id": batch.get("batch_id")})
                    if batch.get("_source") == "local":
                        await mark_batch_retry(batch["batch_id"], str(e))
                    errors += 1
