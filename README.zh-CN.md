...
```bash
# 一行安装脚本（macOS / Linux；Windows 用 install.ps1）
curl -sSL https://weba0.com/install.sh | bash -s -- --proxy

# 或手动安装（--user 自动绕过 Homebrew Python / PEP 668 的限制；
# 如果你已在 venv 里，去掉 --user）：
pip install --user atlast-ecp[proxy]
...
### SDK
- **[Python SDK](sdk/)** — `pip install --user atlast-ecp` — 24 个模块，680+ 个测试
